...
$ini_savedata_num = (ini.get_profile("Game","save_num")).to_i
if $ini_savedata_num == 0 || $ini_savedata_num == ""  #セーブデータ数が0 or ブランク なら最低の1にする
  $ini_savedata_num = 30
end
...
class Window_SaveFile
  ...
  def initialize(file_index, filename)
    ...
    super(0, 56 + file_index % $EXMNU_INCSF_FILE_MAX * wh, 640, wh)
...
class Scene_File
  ...
  def start
    ...
    wh = EXMNU_INCSF_WINDOW_HEIGHT
    adj = 0
    #adj = (480 - @help_window.height) % wh
    #@help_window.height += adj
    @page_file_max = ((480) / wh).truncate
    #@page_file_max = ((480 - @help_window.height) / wh).truncate
    for i in 0...@file_max
