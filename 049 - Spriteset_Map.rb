...
class Spriteset_Map
  ...
  def create_tilemap
    ...
    if $game_variables[17] == 0 || $game_variables[17] == 1
      @tilemap.bitmaps[0] = Cache.system("TileA1")
    else
      @tilemap.bitmaps[0] = Cache.system("TileA1_" + $game_variables[17].to_s)
    end
  ...
  def update_parallax
    if @parallax_name != $game_map.parallax_name
      ...
      if @parallax_name == "ダミー(色置換用)"
        @parallax.bitmap = Cache.parallax(@parallax_name)
        color = set_skn_color 0
        @parallax.bitmap.fill_rect(0,0,64,64,color)
  ...
  def update_viewports
    ...
    #@viewport2.tone = $game_map.screen.tone
    @viewport2.color = $game_map.screen.flash_color
    if $game_switches[22] == false
      @viewport2.ox = $game_map.screen.shake
    elsif $game_switches[22] == true
    ...
    @viewport3.ox = $game_map.screen.shake
    @viewport1.update
    @viewport2.update
    @viewport3.update
  end
  ...
  # ● 自キャラの追加処理
  #--------------------------------------------------------------------------
  def create_eve_chr(x, y,bitmap_name,bitmap_rect)
    i = @chr_sprite.index(nil)
    i = @chr_sprite.size if i == nil
    @chr_sprite[i] = Sprite_Event.new(@viewport2,x, y,bitmap_name,bitmap_rect,1)
  end
end
