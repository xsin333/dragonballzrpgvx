...
class Game_Actor < Game_Battler
  ...
  def level_up
    ...
    #ZP取得
    $zp[@actor_id] = 0 if $zp[@actor_id] == nil
    
    #過去最高レベルの場合ZPを取得
    $cha_maxlv[@actor_id] = 0 if $cha_maxlv[@actor_id] == nil #nil対策
    if $cha_maxlv[@actor_id] < @level
      $zp[@actor_id] += $levelup_add_zp
      $cha_maxlv[@actor_id] = @level
    end
