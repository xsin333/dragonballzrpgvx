...
class Game_Interpreter
  ...
  def update
    loop do
      ...
      if @wait_count > 0                    # ウェイト中
        ...
        #イベントウェイトカット
        if $game_switches[882] == true && @wait_count > 2
          @wait_count = 1
        end
        return
      end
  ...
  def setup_num_input(params)
    if $game_message.texts.size < 4
      ...
      if params[1] == 8 #共有経験値の場合(うまく呼び出せないので直書きする)
        params[1] = 12
      end
  ...
  def command_111
    ...
    when 11  # ボタン
      ...
      if @params[1] == 13 #Cの場合はトリガー
        result = Input.trigger?(@params[1])
      elsif $game_switches[98] == true || $game_switches[9] == true || $game_switches[5] == true && $game_switches[14] == true
  ...
  def command_231
    ...
    #p @params[0],@params[1], @params[2],x, y, @params[6], @params[7], @params[8], @params[9]
...
#=begin
    #ブルマを2つ入れないと表示されないのでとりあえず2つセットする
    z1_cardlist = [9,9,10,12,13,19,20,21,22,23,24,25,26,28,29,30,34,35,36,45,49,50,70,72,75,76,80,82,83,84,87,88,96,100,121,90,103,198,199,200,211,212]
    z2_cardlist = [11,14,15,27,32,44,47,53,54,60,64,154,119,157,169,170,202]
    z3_cardlist = [16,40,41,43,69,79,97,110,63,68,171]
    z4_cardlist = [17,33,46,51,52,61,62,89,104,140,145]
    zG_cardlist = [18,42,93,98,105,106,156]
    zG_add_cardlist = [38]

    #テスト用
    #z3_cardlist.concat([1])

    shoplist = []
    shoplist_order = []
    shoplist.concat(z1_cardlist)

    #shoplist.concat(z2_cardlist)

    #shoplist.concat(z3_cardlist)
    
=begin
    #ここで変更すると何故かイベントが2買い呼ばれるので、イベント側で値を変更する
    #クリアしてたらZG追加カードまで買えるように
    if $game_switches[860] == true
      $game_variables[230] = 11
    end
=end
    if $game_variables[230] == 3 #Z2
      shoplist.concat(z2_cardlist)
    elsif $game_variables[230] == 5 #Z3
      shoplist.concat(z2_cardlist)
      shoplist.concat(z3_cardlist)
    elsif $game_variables[230] == 7 #Z4
      shoplist.concat(z2_cardlist)
      shoplist.concat(z3_cardlist)
      shoplist.concat(z4_cardlist)
    elsif $game_variables[230] == 9 #ZG
      shoplist.concat(z2_cardlist)
      shoplist.concat(z3_cardlist)
      shoplist.concat(z4_cardlist)
      shoplist.concat(zG_cardlist)
    elsif $game_variables[230] == 11 #ZG追加
      shoplist.concat(z2_cardlist)
      shoplist.concat(z3_cardlist)
      shoplist.concat(z4_cardlist)
      shoplist.concat(zG_cardlist)
      shoplist.concat(zG_add_cardlist)
    end
    
    #ztest_cardlist = [91,92,96,99,100,101,107,108,111,112,114,115,116,117,118,119,122,123,142,143,144,145,146,147,148,149,150,151,152,153,154]
    #shoplist.concat(ztest_cardlist)
    
    #ブロッコ倒したらブロッコカードを追加 860は周回プレイ中
    shoplist.concat([194]) if $game_switches[44] == true || $game_switches[860] == true

    #ナッパ倒したらナッパカードを追加 860は周回プレイ中
    shoplist.concat([144]) if $game_switches[67] == true || $game_switches[860] == true
    
    #ウィロー倒したらコーチンカードを追加
    shoplist.concat([187]) if $game_switches[520] == true || $game_switches[860] == true

    #ギニュー倒したらジースカードを追加 860は周回プレイ中
    shoplist.concat([207]) if $game_switches[528] == true || $game_switches[860] == true
    
    #たった一人の最終決戦編をクリアしたら悟空Cカードを追加
    shoplist.concat([184]) if $game_switches[504] == true || $game_switches[860] == true
    
    #魔境星編をクリア(サイボーグフリーザ、コルド来襲)したらコルドカードを追加
    shoplist.concat([195]) if $game_switches[545] == true || $game_switches[860] == true
    
    #クウラ倒したらスカイカー
    shoplist.concat([173]) if $game_switches[533] == true || $game_switches[860] == true
    
    #リキニュウム手に入れたら、パイレ人 586はZG外伝クリア
    shoplist.concat([180]) if $game_switches[828] == true || $game_switches[860] == true || $game_switches[586] == true

    #海賊船を追いかけてオウター星に行ったら、海賊 586はZG外伝クリア
    shoplist.concat([179]) if $game_switches[834] == true || $game_switches[860] == true || $game_switches[586] == true

    #海賊船を追いかけてオウター星に行ったら、オウター人 586はZG外伝クリア
    shoplist.concat([181]) if $game_switches[834] == true || $game_switches[860] == true || $game_switches[586] == true

    #アラレ倒したら、586はZG外伝クリア
    shoplist.concat([201]) if $game_switches[854] == true || $game_switches[860] == true

    #変数を初期化するとアリーナのところで影響があるから、飽くまでnilチェックのみとする
    
    #バトルアリーナでフリーザサイボーグを倒したらフリーザサイボーグカードを追加
    chknum = 28
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([155]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([155]) if $btl_arena_fight_rank[29] == true
    end
    #バトルアリーナで17ごう／18ごうを倒したらピラフカードを追加
    chknum = 32
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([73]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([73]) if $btl_arena_fight_rank[33] == true
    end
    #バトルアリーナで13号を倒したら13号カードを追加
    chknum = 42
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([162]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([162]) if $btl_arena_fight_rank[43] == true
    end
    #バトルアリーナでブロリーを倒したらブロリーカードを追加
    chknum = 48
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([165]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([165]) if $btl_arena_fight_rank[49] == true
    end
    #バトルアリーナでボージャックを倒したらボージャックカードを追加
    chknum = 49
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([166]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([166]) if $btl_arena_fight_rank[50] == true
    end
    #バトルアリーナでチルドを倒したらチルドカードを追加
    chknum = 50
    if $btl_arena_fight_rank_clear_num[chknum] != nil
      shoplist.concat([167]) if $btl_arena_fight_rank_clear_num[chknum] > 0
      #shoplist.concat([167]) if $btl_arena_fight_rank[51] == true
    end
    #クリアしてたら引き寄せと追い払いを買えるように
    if $game_switches[860] == true
      shoplist.concat([102]) #引き寄せ
      shoplist.concat([65]) #追い払い
      #shoplist.concat([196]) #デストロンガス発生装置
    end

    #お気に入りカード追加
    if $game_variables[228] >= $game_variables[229] && $game_switches[469] == true
      shoplist.concat($favorite_card_no)
    end
    
    #芭蕉扇がなければ一定条件で追加する Z3以降、チチの撃破数100
    if $game_variables[40] >= 2 && $cha_defeat_num[10] >= 100 && shoplist.index(140) == nil
      shoplist.concat([140])
    end
    ...
    shoplist_card_num = 0
    #とりあえず格納
    for x in 1..shoplist.size-1
      
      temp_possession_card_id[temp_possession_card_num] = shoplist[x]
      temp_possession_card_order_id[temp_possession_card_num] = $data_items[shoplist[x]].speed
      temp_possession_card_num += 1
    end
      #p temp_possession_card_id,temp_possession_card_order_id
    #並び替え
    #temp_possession_card_num
    for x in 0..$data_items.size
      for z in 0..temp_possession_card_num-1
        
        if x == temp_possession_card_order_id[z]
          shoplist_zen[shoplist_card_num] = temp_possession_card_id[z]
          shoplist_card_num += 1
          $game_temp.shop_goods.push([0,temp_possession_card_id[z]])
        end
      end
    end
    
    #お気に入りがあれば、お気に入りとブルマの並び替え
    if $game_variables[228] >= $game_variables[229] && $game_switches[469] == true
      $game_temp.shop_goods[0],$game_temp.shop_goods[1] = $game_temp.shop_goods[1],$game_temp.shop_goods[0]
    end
...
=begin
    loop do
      ...
      if @list[@index].code == 605          # ショップ 2 行目以降
        ...
        #お気に入りカード追加
        if $game_variables[228] >= $game_variables[229] && $game_switches[469] == true
          $game_temp.shop_goods.push([0,$favorite_card_no])
        end
...
=end    
  ...
  def command_315
    ...
    iterate_actor_id(@params[0]) do |actor|
      ...
      #p actor,@params[0],@params[1],@params[2],@params[3],@params[4]
    end
    return true
  end
  #--------------------------------------------------------------------------
