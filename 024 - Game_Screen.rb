...
class Game_Screen
  ...
  #     duration : 時間
  #--------------------------------------------------------------------------
  def start_tone_change(tone, duration, mode = 0)
    ...
    if mode == 0
      ...
      #@tone_target.red = 128
      #@tone_target.green = 0
      #@tone_target.blue = 255
      #@tone_target.gray = 0 
      result = set_skn_color 0
      result = result.to_s
      result = result[1..result.size-2]
      hairetu = result.split(",")
      for x in 0..2
        hairetu[x] = hairetu[x].to_i
      end
      @tone_target.red = hairetu[0]
      @tone_target.green = hairetu[1]
      @tone_target.blue = hairetu[2]
      @tone_target.gray = 0
    else
      @tone_target.red = 0
      @tone_target.green = 0
  ...
  def update_shake
    ...
    #強制超サイヤ変身揺れ再現
      ...
      tate_shake = 8
      if @shake_duration >= 1 #シェイク時間が残ってるか
        if $game_variables[304] <= 2
