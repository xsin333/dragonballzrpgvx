...
class Window_Selectable < Window_Base
  ...
  def item_rect(index)
    ...
    rect.height = WLH+4
    #rect.x = $put_item_loop_count % @column_max * (rect.width + @spacing)-4
    #rect.y = $put_item_loop_count / @column_max * WLH
    rect.x = index % @column_max * (rect.width + @spacing)-4
...
begin  
  ...
  def output_cursor
    ...
    if $game_switches[73] == true
      ...
      when 3 #ショップ売る
        ...
        x = @index % 3
        y = @index / 3
        self.contents.blt(204*x,8+24*y,picture,rect)
      else #買う売る辞めるだけの想定
        self.contents.blt(6+212*@index,8,picture,rect)
      end
    else
      self.contents.blt(0,@index*24+8,picture,rect)
    end
  end
end
  #--------------------------------------------------------------------------
  ...
  def update_cursor
    if @index < 0                   # カーソル位置が 0 未満の場合
      ...
      when 0 #ショップではない
        self.contents.fill_rect(0,0,16,4800,self.get_back_window_color)
      when 1 #ショップの買う売る辞める選択
        #買う
        self.contents.fill_rect(212*0,8,24,16,self.get_back_window_color)
        #売る
        self.contents.fill_rect(212*1-16,8,24+16,16,self.get_back_window_color)
        #辞める
        self.contents.fill_rect(212*2-16,8,24+16,16,self.get_back_window_color)
      when 2 #ショップ買う
        self.contents.fill_rect(0,0,16,9600,self.get_back_window_color)
      when 3 #ショップ売る
        self.contents.fill_rect(204*0,0,16,6400,self.get_back_window_color)
        self.contents.fill_rect(204*1,0,16,6400,self.get_back_window_color)
        self.contents.fill_rect(204*2,0,16,6400,self.get_back_window_color)
      end
      output_cursor
    end
  end
