...
class Window_Base < Window
  ...
  def initialize(x, y, width, height)
    super()
    if $skin_kanri != nil 
      if $game_switches[1321] == false #矢印なしスキン
        self.windowskin = Cache.system($skin_kanri + "Window")
      else
        self.windowskin = Cache.system($skin_kanri + "Window_arrow")
      end
    else
  ...
  def draw_item_name(item, x, y, enabled = true)
    if item != nil
      ...
      self.contents.blt(x + 24, y,  $tec_mozi,rect)
    end
  end
  #--------------------------------------------------------------------------
  # ● 通貨単位つきの数値描画
  ...
  def draw_currency_value(value, x, y, width)
    ...
    self.contents.font.color = text_color(15)
    #self.contents.draw_text(0, 24, 100, 40, value, 2)
    self.contents.font.color = text_color(15)
    #self.contents.draw_text(0, 0, 100, 40, Vocab::gold, 2)
    #self.contents.font.color.set( 0, 0, 0)
    mozi = value.to_s
    output_mozi mozi
    rect = Rect.new(16*0,16*0, 16*mozi.split(//u).size,24)
    self.contents.blt(80-mozi.split(//u).size*16, 24,  $tec_mozi,rect)
    mozi = Vocab::gold
    output_mozi mozi
    rect = Rect.new(16*0,16*0, 16*mozi.split(//u).size,24)
