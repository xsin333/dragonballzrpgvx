...
class Game_Character
  ...
  attr_reader   :walk_anime               # 歩行アニメか判定
  ...
  #--------------------------------------------------------------------------
  def update_stop
    if @step_anime && @direction_fix && @through == false
      @anime_count += 0.55
    elsif @step_anime == true && @move_speed == 6
      @anime_count += 2.5
    elsif @step_anime
      @anime_count += 1
  ...
  def move_type_custom
    if stopping?
      ...
      if command.code == 0                            # リストの最後
        ...
        when 14   # ジャンプ
          #ジャンプ量xを計算
          #if $game_variables[1] < $game_variables[5]
            jump_x = $game_variables[5] - $game_variables[1]
          #else
          #  jump_x = $game_variables[1] - $game_variables[5]
          #end
          
          #if $game_variables[2] < $game_variables[6]
            jump_y = $game_variables[6] - $game_variables[2]
          #else
          #  jump_y = $game_variables[2] - $game_variables[6]
          #end
          jump(jump_x, jump_y)
